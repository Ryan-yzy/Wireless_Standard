...
$$
C=\{W_0,W_1,\ldots,W_{N_{\mathrm{cb}}-1}\}.
$$

UE 在码本中寻找一个与真实最优方向最接近、或者能带来最好链路性能的码字。这个选择可以抽象为：

$$
W_{\mathrm{opt}}=\arg\max_{W\in C} f(H,W).
$$

这里的 $f(H,W)$ 可以是接收 SNR、吞吐量、SINR、容量近似，也可以是与右奇异子空间距离相关的度量。随后 UE 不需要回传 $W_{\mathrm{opt}}$ 的矩阵元素，只需要回传它在码本中的索引：

$$
\mathrm{PMI}=\mathrm{index}(W_{\mathrm{opt}}).
$$
...
$$
\mathbf H_{\mathrm{eff}} = \mathbf H_{\mathrm{phy}}\mathbf F_{\mathrm{PEB}},
$$

其中 $\mathbf H_{\mathrm{phy}}$ 是物理阵列信道，$\mathbf F_{\mathrm{PEB}}$ 是 gNB 侧从物理天线到 CSI-RS ports 的映射或预波束形成矩阵。UE 基于 $\mathbf H_{\mathrm{eff}}$ 选 PMI，而不是直接基于 $\mathbf H_{\mathrm{phy}}$。
...
$$
\mathbf w_{\mathrm{reg}} = \frac{1}{\sqrt{\beta}} \begin{bmatrix} \sum_{i=0}^{L-1} a_{1,i}\mathbf v_{l_i,m_i}\\ \sum_{i=0}^{L-1} a_{2,i}\mathbf v_{l_i,m_i} \end{bmatrix}.
$$
...
$$
\mathbf F = \begin{bmatrix} \mathbf v_0 & \mathbf v_1 & \cdots & \mathbf v_{P_{\mathrm{CSI-RS}}/2-1} \end{bmatrix},
$$

那么 UE 看到的是：

$$
\mathbf H_{\mathrm{eff}} = \begin{bmatrix} \mathbf H_1\mathbf F & \mathbf H_2\mathbf F \end{bmatrix}.
$$
...
$$
\mathbf w = \hat{\mathbf W}_s\mathbf w_{\mathrm{PMI}}.
$$

这里 $\mathbf w_{\mathrm{PMI}}$ 是由 PMI 指示的稀疏系数向量。
...
$$
\mathbf W_{\mathrm{R17}} = \hat{\mathbf W}_s \mathbf W_c \hat{\mathbf W}_f^{T}.
$$
...
| Release | 统一表示 | 稀疏对象 |
|---|---|---|
| R15 Type II | $\mathbf w=\mathbf W_s\mathbf w_{\mathrm{PMI}}$ | sparse vector |
| R16/R17 Type II | $\mathbf W=\mathbf W_s\mathbf W_{\mathrm{PMI}}\mathbf W_f^{T}$ | sparse matrix |
...
$$
P_{\mathrm{CSI-RS}}\le 32
$$

扩展到：

$$
P_{\mathrm{CSI-RS}}\in\{48,64,128\}.
$$
...
- $P_{\mathrm{tot}}$：聚合后的总 CSI-RS port 数，例如 48、64、128；
...
$$
\{0,1,\ldots,P_{\mathrm{tot}}-1\} \rightarrow P_{\mathrm{set}} \rightarrow \{0,1,\ldots,P_{\mathrm{eff}}-1\},
$$

其中：

- $P_{\mathrm{tot}}$ 是聚合后的总端口数；
- $P_{\mathrm{set}}$ 是被启用的端口子集；
- $P_{\mathrm{eff}}$ 是 UE 实际用于 PMI 推导的有效端口数。

这个选择过程可以用一个选择矩阵表示：

$$
\mathbf h_{\mathrm{eff}} = \mathbf S_{P_{\mathrm{set}}}\mathbf h,
$$

其中 $\mathbf h$ 是完整 large-port effective channel，$\mathbf S_{P_{\mathrm{set}}}$ 只保留被启用的端口。
...
$$
\mathbf W = \hat{\mathbf W}_s^{\mathrm{port}} \mathbf W_c \hat{\mathbf W}_f^T,
$$

其中 $\hat{\mathbf W}_s^{\mathrm{port}}$ 是从 beam-domain CSI-RS ports 中选出的 standard basis。R19 refined port-selection 则是在更大的聚合 port 集合上继续做 port-domain 稀疏选择。
...
$$
\text{space-frequency-time compression} \quad P_{\mathrm{CSI-RS}}\le 32 \rightarrow P_{\mathrm{CSI-RS}}\in\{48,64,128\}.
$$
...
$$
N_{\mathrm{RE}}^{\mathrm{CSI-RS}} \propto P_{\mathrm{tot}} \cdot N_{\mathrm{RB}} \cdot \rho \cdot N_{\mathrm{occasion}}.
$$

当 $P_{\mathrm{tot}}$ 从 32 增加到 128 时，若其他条件不变，端口维度已经增加 4 倍。因此 R20 讨论更低 CSI-RS density 的意义在于：
...
$$
\mathbf z = f_{\mathrm{UE}}(\mathbf H_{\mathrm{eff}}),
$$

$$
\hat{\mathbf W}\ \text{or}\ \hat{\mathbf H} = g_{\mathrm{gNB}}(\mathbf z).
$$

其中：

- $f_{\mathrm{UE}}(\cdot)$ 是 UE 侧 encoder；
- $g_{\mathrm{gNB}}(\cdot)$ 是 gNB 侧 decoder；
...
$$
\text{R15 Type II} : \mathbf w=\mathbf W_s\mathbf w_{\mathrm{PMI}}
$$

$$
\text{R16/R17} : \mathbf W=\mathbf W_s\mathbf W_{\mathrm{PMI}}\mathbf W_f^T
$$
...
$$
\text{R19} : P_{\mathrm{CSI-RS}}\le 32 \rightarrow P_{\mathrm{CSI-RS}}\in\{48,64,128\}
$$

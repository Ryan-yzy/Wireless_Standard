...
$$
\tau_{\max}-\tau_{\min}\leq T_{\mathrm{cp}}.
$$
...
$$
\tau_q\leq T_{\mathrm{cp}}.
$$

对应的 CP 保护距离为

$$
R_{\mathrm{cp}}=\frac{cT_{\mathrm{cp}}}{2}.
$$
...
+
\underbrace{y^{\mathrm{ISI}}_{n,m,q}}_{\text{符号间干扰}}
-
\underbrace{y^{\mathrm{ICI}}_{n,m,q}}_{\text{子载波间干扰}}.
$$
...
其中 $Y_{\mathrm{free}}$ 保留理想距离-多普勒外积结构，而 $Y_{\mathrm{ISI}}$ 与 $Y_{\mathrm{ICI}}$ 会引入符号维和子载波维的耦合。
...
$$
Y=Y_{\mathrm{free}}+Y_{\mathrm{ISI}}-Y_{\mathrm{ICI}}+Z.
$$
...
\Phi_q
\left(
b(\tau_q-T_{\mathrm{cp}})c^H(f_{d,q})\odot S
\right)J_1,
$$
...
=
\frac{
\mathbb{E}\{\|Y_{\mathrm{free}}\|_F^2\}
}{
\mathbb{E}\{\|Y_{\mathrm{ISI}}-Y_{\mathrm{ICI}}\|_F^2\}
+
\mathbb{E}\{\|Z\|_F^2\}
}.
$$
...
\sim \mathcal{CN}(0,P_{\mathrm{ICI}}),
$$

$$
I_s'[i]=\frac{I_s[i]}{d_{i,m}}
\sim \mathcal{CN}(0,P_{\mathrm{ISI}}).
$$
...
$$
\widetilde I_c[p]\sim
\mathcal{CN}\left(0,\frac{P_{\mathrm{ICI}}}{N}\right),
$$

$$
\widetilde I_s[p]\sim
\mathcal{CN}\left(0,\frac{P_{\mathrm{ISI}}}{N}\right).
$$
...
当子载波数 $N$ 足够大时，$\frac{I_t}{N}$ 可能较小。此时最大感知距离主要受路径损耗、目标 RCS、处理增益和检测阈值影响，而不应简单等同于 $cT_{\mathrm{cp}}/2$。
...
$$
t=mT_s+vT_{\mathrm{cp}}
$$

开始，主要检测满足

$$
vT_{\mathrm{cp}}\leq \tau <(v+1)T_{\mathrm{cp}}
$$
...
$$
[0,T_{\mathrm{cp}}),
[T_{\mathrm{cp}},2T_{\mathrm{cp}}),
[2T_{\mathrm{cp}},3T_{\mathrm{cp}}),\ldots
$$
...
y_m^{(v)}[n]
=
y[n+m(N+N_{\mathrm{cp}})+vN_{\mathrm{cp}}],
\quad
m=0,\ldots,M-1.
$$
...
$$
\hat \tau=\frac{\hat p+vN_{\mathrm{cp}}}{B}.
$$
...
=
\frac{1}{N}
R_m[p-N_{\mathrm{lag}}]_N
e^{j\pi p},
\quad
p=0,\ldots,N_{\mathrm{cp}}+N_{\mathrm{lag}}-1.
$$

其中 $N_{\mathrm{lag}}$ 用于处理 off-grid delay 带来的非因果部分。
...
\sum_{k=0}^{N-1}
d_{k,m}
e^{j\frac{2\pi}{N}k(n+N_{\mathrm{cp}})}.
$$
...
$$
\hat y_m^0[n]=\tilde y_m^0[n+N_{\mathrm{lag}}].
$$
...
$$
\hat Y_{\mathrm{ISI}},\quad \hat Y_{\mathrm{ICI}},
$$
...
Y_{\mathrm{free}}^{k+1}
=
Y-\hat Y_{\mathrm{ISI}}^{k}+\hat Y_{\mathrm{ICI}}^{k}.
$$
...
随后重构 $Y_{\mathrm{ISI}}^{k}$ 与 $Y_{\mathrm{ICI}}^{k}$，并更新：
...
$$
\frac{
\|Y_{\mathrm{free}}^{k+1}-Y_{\mathrm{free}}^{k}\|_F
}{
\|Y_{\mathrm{free}}^{k}\|_F
}
<
...
之后与 SIC-DFT 一样，估计 $\alpha_q$，重构 ISI/ICI，并更新 $Y_{\mathrm{free}}$。
...
| SIC-DFT | 固定窗口下 DFT 估计目标，重构 ISI/ICI | 结构化 $Y_{\mathrm{ISI}},Y_{\mathrm{ICI}}$ | 降低 RDM 旁瓣和结构化泄露 | 易实现，兼容 RDM/CFAR | DFT 栅格限制，off-grid 性能有限 |
| SIC-ESPRIT | 固定窗口下子空间估计目标，重构 ISI/ICI | 结构化 $Y_{\mathrm{ISI}},Y_{\mathrm{ICI}}$ | 高精度参数估计与干扰抵消 | 超分辨，精度高 | 复杂度高，对模型和阶数估计敏感 |
...
$$
Y_{\mathrm{ISI}},\quad Y_{\mathrm{ICI}}.
$$

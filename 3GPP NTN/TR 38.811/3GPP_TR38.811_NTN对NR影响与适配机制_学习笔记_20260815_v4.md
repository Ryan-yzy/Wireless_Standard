---
title: "TR 38.811 NTN 对 NR 影响与适配机制学习笔记 v4"
author: ""
date: "2026-08-15"
---
...
> 本笔记基于 3GPP TR 38.811 V15.4.0（Release 15）Clause 7 整理，并结合 TS 38.211、TS 38.214 的 NR 基线机制解释。全文按“物理特征与同步 → 长时延闭环与随机接入 → 局部信道与 CP → 星上射频约束 → NG-RAN 映射”五个主题展开。TR 中的 Problem statement、Assessment 和 NR impact consideration 分别表示问题提出、条件评估和潜在影响，不能直接等同于已经写入规范的修改。

| 主章节 | 核心比较量 | 主要 NR 机制与原文位置 |
|---|---|---|
| 物理特征、移动性与空口同步 | 波束驻留、\(\dot\tau T\)、残余频偏、\(\dot f_D T\) | 移动性、TA、PSS/SSS、DM-RS；Clauses 7.3.2.1-7.3.2.4 |
| 长传播时延、协议闭环与随机接入 | 单程时延、传播 RTT、协议闭环、差分时延 | HARQ、MAC/RLC、ACM、TDD/FDD、PRACH/RAR；Clauses 7.3.3-7.3.4、7.3.6 |
| 局部信道、参考信号与循环前缀 | 相干带宽、超额时延、CP 覆盖 | DM-RS Type 1/2、SCS、正常/扩展 CP；Clause 7.3.5 |
| 星上载荷、射频损伤与波形约束 | CPE、ICI、OBO、有效 EIRP | PT-RS、PAPR、DFT-s-OFDM；Clause 7.3.7 |
| NG-RAN 架构映射与适配准则 | 功能终止位置、接口跨越链路、机制容限 | CU/DU、F1/N2/N3、移动性和横向边界；Clause 7.3.8 |

---
title: "TR 38.811 NTN 天线方向图与信道模型学习笔记 v7"
author: ""
date: "2026-08-15"
---
...
> 本笔记基于 3GPP TR 38.811 V15.4.0（Release 15）Clauses 6.3-6.10 整理，并以 TR 38.901 Clause 7.5 的几何随机信道模型（Geometry-based Stochastic Channel Model，GSCM）及 Clause 7.7 的链路级模型作为必要补充。正文先建立“通用信道生成流程”，再按“NTN 总体替换关系 → 链路预算与大尺度传播 → LSP 与局部散射 → 天线投影与链路级模型重构”展开。

涉及的 ITU-R 方法按 TR 38.811 所引用的建模入口说明，不把后续新版 ITU-R 参数反向写成 3GPP 原文。

链路几何、传播时延、差分时延、卫星多普勒的计算已在同目录的[《TR 38.811 NTN 基础架构与时频特性学习笔记 v2》](./3GPP_TR38.811_NTN基础架构与时频特性_学习笔记_20260804_v2.md)第 7～11 章完整介绍。本文不再重复推导，只把实际仰角、斜距、几何时延、传播方向和公共多普勒作为信道生成器的外部输入。
...
这里的箭头表示生成和装配顺序，不表示所有模型都必须逐级运行。完整 GSCM、CDL、TDL 和平坦衰落是不同复杂度的局部信道表示；链路预算和天线模型则为它们提供功率尺度与端口响应。
...
> **关键原文定位：**TR 38.901 Clause 7.5 给出完整随机信道生成流程，Clause 7.7 给出 CDL/TDL 及其缩放方法。
...
> **关键原文定位：**TR 38.901 Clauses 7.7.1-7.7.3 给出 CDL 的系数生成、缩放与使用方法。

### 1.6 CDL 到 TDL：固定空间条件后的降维
...
> **关键原文定位：**TR 38.901 Clauses 7.5 和 7.7 分别给出完整生成器与 CDL/TDL；具体场景再决定应填入哪些统计参数和附加传播项。
...
本文从“基础架构与时频特性”笔记接收下列量，不再重复 ECEF/局部坐标变换、斜距、传播时延和卫星多普勒推导：
...
| 几何侧输入 | 在信道模型中的用途 |
|---|---|
| 实际仰角 \(\alpha(t)\) | 选择/映射 LOS、杂波、SF、LSP 参数，设置 ZOD/ZOA 中心并计算天线离轴角 |
| 斜距 \(d(t)\) | 计算 FSPL；不在局部 CDL/TDL 中再次产生 |
| 几何时延 \(\tau_{\mathrm{geo}}(t)\) | 平移所有局部超额时延，形成绝对到达时延 |
| 传播方向 \(\widehat{\mathbf r}(t)\) | 进入卫星、HAPS、UE/VSAT 的方向图和阵列导向矢量 |
| 公共多普勒 \(\nu_{\mathrm{sat}}(t)\) | 作为全部局部路径近似共享的相位旋转，与 UE 侧子径多普勒分开 |
...
e^{j\phi_{\mathrm{sat}}(t)}
\mathbf F_r^H(t)
\mathbf H_{\mathrm{local}}
\left(t,\tau-\tau_{\mathrm{geo}}(t)\right)
\mathbf F_t(t),
\]
...
|---|---|---|---|---|
| 几何输入 | BS/UT 位置、二维/三维距离和四类角度 | 平台轨道或高度、实际仰角、长斜距、几何时延和大公共多普勒；具体推导由另一份笔记承担 | \(d,\alpha,\tau_{\mathrm{geo}},\Omega_{\mathrm{LOS}},\nu_{\mathrm{sat}}\) | Clauses 4-5、6.3 |
...
| 簇与子径 | 生成时延、功率、角度、XPR、相位和多普勒 | 散射主要集中在 UE 周围；平台侧方向高度集中，UE 侧保留局部角扩展 | \(\tau_n,P_n,\Omega_{n,m},\alpha_{n,m}\) | Clause 6.7.2 + TR 38.901 7.5 |
...
4. **绝对时延与超额时延分离。**毫秒级 \(\tau_{\mathrm{geo}}\) 影响同步和协议，纳秒至微秒级 \(\tau_{n,\mathrm{excess}}\) 决定频率选择性；CDL/TDL 表只描述后者。
...
> **关键原文定位：**TR 38.811 Clauses 6.3-6.10；TR 38.901 Clauses 7.1-7.7。
...
这里的 \(\tau_{\mathrm{geo}}(t)\) 与 \(\nu_{\mathrm{sat}}(t)\) 由第 2.2 节所引用的几何侧模型提供；本节只规定它们如何与局部多径组合。
...
几何时延与多普勒的数值由第 2.2 节所引用的几何侧模型给出；在长时信道中，两者的接口关系为：
...
本章的逻辑不是把天线与 TDL 并列，而是沿着实际生成顺序展开：
...
> **关键原文定位：**TR 38.811 Clause 6.4.2 定义圆极化、固定或跟踪的 VSAT 参考天线；Clause 6.7 说明相干带宽取决于环境、天线方向图和仰角。

### 5.3 阵元、端口、波束与接收投影

#### 5.3.1 圆形孔径、端口、波束与数据流
...
### 5.4 天线空间滤波与等效 NTN-TDL

固定发射波束和接收波束后，CDL 中每条路径先按方向图、阵列导向矢量和极化进行加权，再把相同时延或同一簇内的贡献合成为抽头。由此得到的 TDL 隐含了生成它时使用的天线条件，这正是本节与前述天线模型的接口。
...
以两径信道说明方向图对平坦化判据的影响。直达径时延为 0，等功率反射径相对时延为 \(1\ \mu\mathrm{s}\)：
...
有效 RMS 时延扩展约降为 \(0.099\ \mu\mathrm{s}\)。这时仍应使用

\[
B_{\mathrm{sig}}\sigma_{\tau,\mathrm{eff}}\ll1
\]

判断是否可平坦化：同一 VSAT 在 1 MHz 业务下可能使用单抽头，在 100 MHz 业务下仍可能需要 TDL 或 CDL。
...
所以从 CDL 到 TDL 的关系应理解为：

\[
\boxed{
\text{CDL 角度路径}
+\text{目标天线/波束}
\xrightarrow{\text{空间滤波}}
\text{等效 TDL}
}
\]

标准 NTN-TDL 采用规定的空间滤波基线，适合不研究角度和波束变化的链路级问题；若改变 UE/VSAT 方向图、波束宽度或指向误差，应回到 CDL 应用目标天线，再生成新的等效 TDL。

> **关键原文定位：**TR 38.811 Clauses 6.4.2、6.7 和 6.9.2；TR 38.901 Clauses 7.7.2-7.7.4 给出通用 TDL 与空间滤波方法。

### 5.5 归一化时延与 NTN-A～D 参考剖面
...
例如 \(\widehat\tau_p=2.5\)、\(DS_{\mathrm{target}}=100\ \mathrm{ns}\)，则该抽头局部超额时延为 \(250\ \mathrm{ns}\)。这仍不包含第 2.2 节所引用的毫秒级几何传播时延。

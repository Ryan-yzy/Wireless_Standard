...
> 本笔记把 NTN 的时延、多普勒、波束和局部信道特征映射到 NR 同步、随机接入、调度、反馈、参考信号、双工和移动性机制。每节区分研究报告中的影响/候选方案、规范机制和工程解释。

| 主章节 | 核心对象 | 主要来源 |
|---|---|---|
| 同步与时序 | 初始同步、残余频偏、TA、K_offset | TR 38.811/38.821 |
| 接入与闭环 | PRACH/RAR、HARQ/RLC、调度与功控 | TR 38.811 Clause 7 |
| 参考信号与波形 | DM-RS、PT-RS、CP、PAPR、功放 | TR 38.811 Clause 7 |
| 移动性与架构 | 切换、载荷和 NG-RAN 接口 | TR 38.811 Clauses 7.3.7-7.3.8 |
...
平台运动、斜距变化与几何多普勒来自同一轨道运动，却跨越不同时间尺度：移动性关注分钟级波束驻留，TA 关注更新周期内的时延漂移，初始同步关注预补偿后的残余频偏，DM-RS 则关注一个时隙内的剩余变化。
...
> **原文定位：**TR 38.811 Clause 7.3.2.1。报告认为 GEO 和多数 HAPS 场景不产生同等程度的协议影响；NGSO 的切换与寻呼适配需进一步研究。
...
> **原文定位：**TR 38.811 Clause 7.3.2.2、Table 7.3.2.2.2-1。报告结论是需要研究可预测 NTN 时延下的上行对齐方案，而不是认为 TA 必须在单个 TTI 内完成全部补偿。
...
> **原文定位：**TR 38.811 Clause 7.3.2.3。报告给出的约 13,000 km 高度判断来自其 \(5\,\mathrm{ppm}\) 与最大几何多普勒比较，不是通用系统部署门限。

### 1.5 多普勒变化率与 DM-RS 时间跟踪
...
> **原文定位：**TR 38.811 Clause 7.3.2.4、Table 7.3.2.4.1-1。公共、残余和信息老化的分解属于基于原文条件的接收机工程解释。
...
> **原文定位：**多普勒与变化率见 Clause 5.3.1.3、Clause 5.3.4.3-5.3.4.4；预补偿与初始同步见 Clause 7.3.2.3；DM-RS 时域密度分析见 Clause 7.3.2.4、Table 7.3.2.4.1-1。公共/残余/差分频偏的拆分为基于上述条件的接收机工程解释。
...
> **原文定位：**TR 38.821 Clause 6.2.1、Figure 6.2.1-1 和 Figure 6.2.1-2。
...
> **原文定位：**TR 38.821 Clause 6.2.1；TS 38.214 Clause 5.2、6.1.2.1.1 和 6.2.1；TS 38.213 Clause 9。
...
> **原文定位：**TR 38.811 Clause 7.3.3.1，Figures 7.3.3.1-1、7.3.3.1.1-1，Table 7.3.3.1.1-1。表中数值是研究评估用参考，不是所有 NTN 系统的强制 HARQ 进程配置。

#### 5.2.2 HARQ 与 RLC ARQ
...
### 5.3 MAC/RLC 调度与链路自适应

#### 5.3.1 MAC/RLC 缓存、定时器与调度
...
> **原文定位：**TR 38.811 Clause 7.3.3.2。原文结论是 ARQ 协议本身不一定改变，但 buffer、timeout、重传次数和上行调度时序需按 NTN 长时延重新定尺寸。

#### 5.3.2 ACM 与功率控制的反馈老化
...
> **原文定位：**TR 38.811 Clause 7.3.3.3；MCS和物理层调度基线参见 TS 38.214 Clauses 5.1.3、5.2.2 和 6.1.4。TR 只要求进一步研究功控和 ACM 闭环所需裕量，没有规定上述工程分解的数值。
...
> **原文定位：**TR 38.811 Clause 7.3.6.1。\(T_{\mathrm{guard}}\approx2\tau\) 是报告采用的传播级保守估计，不是包含处理与调度的完整协议 RTT。

### 5.5 PRACH 时延组成与接收窗口

#### 5.5.1 公共时延、差分时延与残余时延
...
> **原文定位：**TR 38.811 Clause 7.3.4.1.1、Figure 7.3.4.1.1-1、Table 7.3.4.1.2-1。

#### 5.5.2 PRACH 检测、RAR 窗口与 TA
...
> **原文定位：**TR 38.811 Clauses 7.3.4.1.2-7.3.4.1.3。

### 5.6 RAR 时序与 TA 范围

#### 5.6.1 RAR 响应中的上下行传播时延
...
> **原文定位：**TR 38.811 Clause 7.3.4.1.2。GEO弯管场景的 RAR 往返量级可达约600 ms，报告因此认为 RAR窗口及其节能设计需重新评估。
...
> **原文定位：**TR 38.811 Clauses 7.3.4.2.1-7.3.4.2.3，Tables 7.3.4.2.1-1、7.3.4.2.2-1。表格是报告的参考评估，具体 Rel-17/后续 NTN TA 机制应以相应 TS 版本为准。
...
> **原文定位：**TS 38.213 Clause 7.1.1。

### 6.2 Layer 3 滤波与 NTN 适应性
...
> **原文定位：**TR 38.821 Clause 6.2.2；TS 38.331 Clause 5.5.3.2。
...
#### 7.1.2 DM-RS 频域配置 Type 1 与 Type 2
...
> **原文定位：**TR 38.811 Clause 7.3.5.1.1、Tables 7.3.5.1.1-1 至 7.3.5.1.1-2；精确RE和端口映射参见 TS 38.211 Clause 7.4.1.1.2。
...
> **原文定位：**TR 38.811 Clauses 7.3.5.1.1-7.3.5.1.3、Table 7.3.5.1.1-3。
...
> **原文定位：**TR 38.811 Clause 7.3.7.1、Annex B。报告的“现有设计可补偿”带有明确条件，不能扩展成所有 NTN 相位噪声与多普勒场景都无影响。

### 8.2 功放非线性、PAPR 与波形选择

#### 8.2.1 PAPR、功放非线性与 OBO
...
> **原文定位：**TR 38.811 Clause 7.3.7.2、Figure 7.3.7.2.1-1。总损失数值来自报告引用的 ETSI TR 103 297 示例。
...
前述各节分别讨论移动、时延、波束、多径、硬件和架构，本节只做横向收敛。共同判断方法是先识别可预测的几何公共量，再确定差分量和残余误差，最后把它们与相应机制的采样周期、反馈周期、范围或硬件裕量比较。不同功能看似分散，实际反复落在确定性公共量、剩余动态误差和星上资源约束三类根因上。
...
> **原文定位：**TR 38.811 Clause 7.3.8.1.2，Figures 7.3.8.1.2-1 至 7.3.8.1.2-3，Tables 7.3.8.1.2-1 至 7.3.8.1.2-2。
...
> **原文定位：**TR 38.811 Clauses 7.3.8.1.1-7.3.8.1.3、Table 7.3.8.1.2-3。
...
> **原文定位：**TR 38.811 Clauses 7.3.2-7.3.8 分别讨论平台运动、长传播时延、大波束、传播信道、双工、星上硬件和 NG-RAN 架构映射；本表用于横向比较，不改变各小节的标准状态判断。

## 11 协议状态向系统仿真的输出接口

| 状态 | 更新依据 | [系统仿真](./06_NTN链路系统与多星仿真_学习笔记.md)中的处理 |
|---|---|---|
| TA、公共/差分/残余时延 | 几何、补偿和测量 | 到达窗口与上行对齐 |
| \(K_{\mathrm{offset}}\) 与跨 DL-UL 时序 | 配置与过程触发 | 调度、反馈和参考资源时间轴 |
| 公共/残余/差分多普勒 | 星历、位置、估计和补偿时效 | 捕获范围、残差模型和 BLER 映射 |
| CSI/RSRP/功控状态 | 参考信号、滤波和反馈 RTT | 老化、预测、裕量和发射功率 |
| HARQ/RLC/队列状态 | ACK/NACK、定时器和缓存 | 进程占用、重传与时延统计 |
| Beam/Cell/Satellite 状态 | 测量、预测和移动性控制 | 多速率切换状态机 |

系统级抽象不能把 RTT、CSI 老化或 HARQ 进程占用统一写成固定 SINR 扣减；它们分别作用于时间轴、决策信息和队列状态。只有明确映射假设并用链路级结果校准后，才可把其中一部分折算为等效性能损失。

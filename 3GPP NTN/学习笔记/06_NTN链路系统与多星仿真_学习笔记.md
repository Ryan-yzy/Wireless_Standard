...
## 7 校准、输出指标、统计置信度和模型边界

校准先固定场景、链路预算参考点、天线增益定义、噪声带宽、干扰合并位置和链路抽象曲线，再比较关键中间量，而不是只对齐最终吞吐量。最少输出覆盖/接入成功率、BLER、吞吐量、频谱效率、时延、队列长度、切换次数/失败率、服务中断和干扰分布，并同时报告样本数、随机种子策略、置信区间或分位数。

仿真结论必须随模型层级解释：LLS 证明的是给定链路假设下的收发机性能；单星 SLS 证明的是给定负载、调度和复用下的网络性能；多星结果还依赖星座、服务选择和星间协调。任何一级的结果都不能脱离其输入分布外推为所有 NTN 配置的强制结论。

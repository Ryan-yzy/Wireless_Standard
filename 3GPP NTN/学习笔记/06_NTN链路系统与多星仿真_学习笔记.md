...
本篇沿用[《NTN 系统架构与部署场景》“参考场景与终端类型”](./01_NTN系统架构与部署场景_学习笔记.md#3-参考场景与终端类型)中的定义，不重复终端射频数值。Clause 6.1 评估主要选用 Scenario A（GEO 透明转发）、C2（LEO 透明转发、移动波束）和 D2（LEO 再生载荷、移动波束）；一次仿真配置只需明确声明场景标识、S/Ka 终端 profile、Set-1/Set-2 卫星能力集及其来源。Set-1/Set-2 是成套 RF、口径、波束和 G/T 参数，不是两种算法；不同配置的性能差异也不能只归因于载频。

> **原文定位：**TR 38.821 Clause 6.1.1.1、Tables 6.1.1.1-1 至 6.1.1.1-7；完整架构、频段和终端参数由第一篇统一维护。
...
校准阶段先固定场景、链路预算参考点、方向图、噪声带宽、干扰合并位置和链路抽象曲线，并对齐 Coupling Loss、Geometry SIR/SINR 等中间量。校准使用共同约定的全负载或确定性资源活动掩码，避免业务随机性掩盖几何与干扰差异；性能评估阶段才按 FTP-3、调度结果或声明的负载模型生成活动度并比较接收机、调度器或波束算法。统一输出至少包括覆盖/接入成功率、同步检测概率、BLER、SINR、吞吐量、频谱效率、时延、队列长度、切换或波束失败率、服务中断和干扰分布，并报告样本数、随机种子、置信区间或分位数。
...
“不小于 64”是 Table 6.1.2-2 的研究评估表述；同一报告也说明 NR 基线一次可配置的前导数上限为 64。因此基线实现应取 64；只有在明确评估新格式或扩展设计时，才可把“大于 64”作为额外研究假设，不能写成既有 Rel-15/16 的通用配置能力。
...
#### 3.3.1 Coupling Loss 与链路预算的接口

Coupling Loss 的量项、参考端口和与 \(C/N_0\) 的边界由[《NTN 传播损耗与信道模型》“Coupling Loss 与链路预算的接口”](./04_NTN传播损耗与信道模型_学习笔记.md#32-coupling-loss-与链路预算的接口)统一定义。本篇只接收每条“发射源 \(j\) 到 UE \(u\)”的 \(CL_{u,j}\)，并在调度资源上换算接收功率：
...
CL 不包含发射功率、接收机噪声、同频干扰、编码增益或基带合并增益；系统级仿真必须保证上游已经一次性装配了路径损耗、附加传播损耗、阴影、穿透、极化和收发天线增益，不能在本篇再次叠加。
...
因此本篇保留 \(P_{\mathrm r}=P_{\mathrm t}-CL\) 和 CNR/SINR 的下游换算，不再重复上游 CL 的完整分解与数值例子。

#### 3.3.2 CIR、SIR 与信道建模层次
...
每个调度时刻按“轨道传播与几何筛选 → 服务/切换/队列状态 → 星内与星间链路计算 → L2S、BLER 与 HARQ → 统计更新”顺序执行；进入下一时刻时，只刷新到期的时间尺度。这样既保留各状态的真实时间戳，也避免把整套星座按 OFDM 采样率重复计算。

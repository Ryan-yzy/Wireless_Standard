...
> 本笔记从链路预算参考点出发，装配大尺度传播损耗，再生成局部宽带时变信道，并说明 GSCM、CDL、TDL 与平坦衰落的保留关系。几何和天线响应分别由第二、三篇输入。

| 主章节 | 核心对象 | 主要来源 |
|---|---|---|
| 链路预算 | EIRP、G/T、C/N0、Coupling Loss、SINR | TR 38.811 Clause 5.2 |
| 大尺度传播 | FSPL、杂波、阴影、建筑、大气 | TR 38.811 Clause 6 |
| 局部信道 | LSP、簇、子径、极化和多普勒 | TR 38.811/38.901 |
| 模型降阶 | GSCM、CDL、TDL、平坦衰落 | TR 38.901 |
...
这里的 \(\tau_{\mathrm{geo}}(t)\) 与 \(\nu_{\mathrm{sat}}(t)\) 由[卫星轨道时间与链路几何](./02_卫星轨道时间与链路几何_学习笔记.md)提供；本节只规定它们如何与局部多径组合。
...
#### 5.3.1 阶段一：场景、传播状态与相关大尺度参数（步骤 1～4）
...
#### 5.3.3 阶段三：簇内耦合、XPR 与初相位（步骤 8～10）
...
#### 5.3.4 阶段四：端口系数、LOS 分量与大尺度缩放（步骤 11～12）
...
> **原文定位：**TR 38.811 Clauses 6.4.2、6.7 和 6.9.2；TR 38.901 Clauses 7.7.2-7.7.4 给出通用 TDL 与空间滤波方法。

## 8 归一化时延与 NTN-A～D 参考剖面

...
这里的 \(CL_{\mathrm{clutter}}\) 表示杂波损耗，不是第 3.2 节的 Coupling Loss。步骤 3 只装配给定场景和传播状态下的确定性项或条件中位项，不另行抽取随机阴影衰落；具体项按频段、部署和模型路径开关，不能无条件全部叠加。
...
若前 11 步生成的是单位平均功率的小尺度信道，则把步骤 4 联合抽取的同一个阴影衰落样本装配到总损耗：

\[
PL_{\mathrm{total}}(t)
=
PL_{\mathrm{median}}(t)+SF_{\mathrm{loss}}(t),
\]
...
=
10^{-PL_{\mathrm{total}}(t)/20}
\mathbf H_{\mathrm{small}}(t,\tau).
\]

功率域应使用 \(10^{-PL/10}\)，复幅度域使用 \(10^{-PL/20}\)。因此 SF 只在步骤 4 作为相关 LSP 抽取一次，步骤 12 复用该样本；不能在步骤 3 和步骤 12 分别生成或施加两次。
...
若另一条简化链路预算路径已经独立生成了 \(SF_{\mathrm{loss}}\)，这里不能再生成第二个 SF。完整 GSCM 主链应在本步骤一次性生成相关 LSP，并在步骤 12 复用同一样本；符号转换为：
...
Loo 状态持续参数以距离为单位。只有当状态过程由终端穿越局部遮挡场驱动时，若某状态持续距离为 \(D_{\mathrm{state}}\)，UE 速度为 \(v_{\mathrm{UE}}\)，才可写成：
...
该换算不能直接套用于“静止 UE + 移动 LEO”。即使 \(v_{\mathrm{UE}}=0\)，卫星视线方向仍会扫过建筑、树木或终端附近遮挡边界，传播状态可能继续变化；此时应使用视线在局部遮挡场中的等效扫掠速度，或直接采用几何驱动的状态过程，而不能由分母为零推出“状态永不切换”。
